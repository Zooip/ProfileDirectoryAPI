...
RSpec.describe MasterData::Profile, type: :model do
  
  it "has a valid factory" do
    expect(FactoryGirl.build(:master_data_profile)).to be_valid
  end

  it "has an empty database" do
    expect(MasterData::Profile.count).to eq(0)
  end
  ...
  it "validate presence of :soce_id" do
    profile=FactoryGirl.create(:master_data_profile, create_without_aliases:true)
    profile.soce_id=nil
  ...
  it "destroy its connection_aliases when destroyes" do |variable|
    profile=FactoryGirl.create(:master_data_profile, create_without_aliases:true)
    con_alias=FactoryGirl.create(:master_data_connection_alias, profile: profile)
    profile.destroy
    expect(MasterData::ConnectionAlias.exists?(con_alias.id)).to eq(false)
  end
  ...
  describe "sync email and emergency_email" do
    context "when there is not emergency_email" do
      it "update :emergency_email with value of :email" do
        profile=FactoryGirl.create(:master_data_profile,email:'roland.vardanega@gadz.org', emergency_email:nil, create_without_aliases:true)
        expect(profile.emergency_email).to eq("roland.vardanega@gadz.org")
      end
    end    
    context "when they are the same before the update" do
      it "update :emergency_email with value of :email" do
        profile=FactoryGirl.create(:master_data_profile,email:'roland.vardanega@gadz.org', emergency_email:'roland.vardanega@gadz.org', create_without_aliases:true)
    ...
    context "when they are different before the update" do
      it "doesn't change :emergency_email value" do
        profile=FactoryGirl.create(:master_data_profile,email:'roland.vardanega@gadz.org', emergency_email:'roland211@hotmail.com', create_without_aliases:true)
        profile.update_attribute(:email,"roland@gadz.org")
  ...
  describe "Soce_id auto_increment" do
    ...
    it "auto increment soce_id" do
      account1=FactoryGirl.create(:master_data_profile)
      expect(FactoryGirl.create(:master_data_profile).soce_id).to eq(account1.soce_id+1)
    end
    ...
    describe "update soce_id sequence when user input" do
      context "when user input greater than actual sequence" do
        it "update soce_id next value " do
          account1=FactoryGirl.create(:master_data_profile)
          account2=FactoryGirl.create(:master_data_profile, soce_id: account1.soce_id+10)
          expect(FactoryGirl.create(:master_data_profile).soce_id).to eq(account1.soce_id+11)
        end
      end
      ...
      context "when user input lesser than actual sequence" do
        it "doens't update soce_id sequence when user input" do
          account1=FactoryGirl.create(:master_data_profile)
          account2=FactoryGirl.create(:master_data_profile, soce_id: account1.soce_id+10)
          account3=FactoryGirl.create(:master_data_profile, soce_id: account1.soce_id+5)
          expect(FactoryGirl.create(:master_data_profile).soce_id).to eq(account1.soce_id+11)
        end
      end
    end
